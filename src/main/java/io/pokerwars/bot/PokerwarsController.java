...
public class PokerwarsController {
  ...
  public ResponseEntity<PokerAction> play(@RequestBody GameInfo gameInfo) {
    ...
    if (gameInfo.canCheckOrBet()) {
      ...
      LOG.info("In this hand, your bot can check or bet");
      LOG.info("If you bet, the minimum bet is {}", gameInfo.getMinBet());
    }

    if (gameInfo.canCallOrRaise()) {
      // remember: in poker you can call or raise only if there has been a bet before
      LOG.info("In this hand, your bot can call or raise");
      LOG.info("If you call, you'll spend {} chips", gameInfo.getChipsToCall());
      LOG.info("If you raise, the minimum raise is {}", gameInfo.getMinRaise());
    }
